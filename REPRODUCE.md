...
- **Extracted illuminance** (`data/lux_csv/lux_Location_<X>.csv`, `timestamp;lux`) and the
  **Thissio pyranometer** record. The synthetic lux CSVs are included under `data/synthetic_lux/`; the Thissio pyranometer data are owned by the National Observatory of Athens and available on reasonable request.
...
#     NOTE: no --geometry-dir -> the clean horizontal-open result (M2 R^2=0.963)
...
uv run python scripts/make_paper_figures.py --pooled data/dataset/lux_ghi_monolithic.csv \
    ...
    --out-dir data/results/figures   # F1-F6; F7/F9b are composed from the released Isaac screenshots via make_j_occlusion_figure.py
```

Expected headline numbers (clear-sky): per-site calibration R² 0.85–0.98; physical-model
leave-location-out R² **0.963** (linear baseline 0.918 elev>=10 / 0.939 all-daylight); luminous efficacy 27.1–32.2 lm/W
...
- `scripts/render_lux_map_gui.py`: full-frame lux map (`.npy`).
- `scripts/extract_geometry_gui.py`: per-pixel tilt/SVF/horizon (optional; the azimuth convention is discussed as a limitation in the paper).
