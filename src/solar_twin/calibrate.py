...
GHI), per location. Two framings:

  A) physical efficacy:  GHI = lux / K_site         (alpha = 1/K, beta ~ 0)
  B) empirical OLS:      GHI = a_site * lux + b_site (fitted freely)

Initial tests (Location_A) showed lux<->GHI is tightly linear (R^2~0.976) but the
...
The per-season slope difference (26.5 vs 33.4) is informative.
"""
...
def fit_geometry_aware(lux, ghi, elevation_deg, air_mass):
    ...
    Tests whether geometry features tighten the fit beyond lux alone. coefs order: [intercept, lux, elevation, air_mass].
    """
