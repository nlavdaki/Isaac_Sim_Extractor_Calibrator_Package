r"""Publication figures (uv, matplotlib).
...
def fig_convergence(path, out_dir):
    ...
    # legend outside the axes, bottom-right (below the x-axis); savefig.bbox='tight' keeps it in frame
    ax.legend(handles=handles, loc="upper right", bbox_to_anchor=(1.0, -0.16),
...
def main():
    ...
    ap.add_argument("--lux-map", help="lux_map_<site>.npy -> legacy .npy lux map (the paper F7 is the "
                                      "native Isaac screenshot via make_j_occlusion_figure.py)")
    ...
    if a.lux_map and os.path.exists(a.lux_map):
        try:
            ...
            render_lux_map(a.lux_map, os.path.join(a.out_dir, "lux_map_npy.png"),
                           title=f"RTX path-traced illuminance - {site}")
            print("[fig] wrote lux_map_npy.png / .pdf (paper F7 is the native screenshot)")
        except Exception as e:  # noqa: BLE001
