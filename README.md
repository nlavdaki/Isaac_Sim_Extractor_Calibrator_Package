...
the steep site **H**). Geometry-pass refinement (the surface-azimuth convention) is future
work and is not required for the headline result.
...
|---|--------|-----|----------|------|
| P1 | `benchmark_convergence_gui_v2.py` | Isaac GUI | `convergence_table_spp.csv` — **totalSpp** {8…512} × mean/std lux × render-time × VRAM | once |
...
| P5 | `validate_thissio.py` | uv | `thissio_validation_summary.csv` (+ scatter) — linear / physical / CAMS arms vs Thissio pyranometer | after `build_dataset` |
| P6 | `ablation_report.py` | uv | `ablation_loo.csv` — M0→M4 physical-model leave-location-out ablation | after `build_dataset` |
| P7 | `extract_geometry_gui.py` | Isaac GUI | `geometry_<site>.json` — tilt/azimuth/SVF/horizon per pixel | per scene |
...
The `uv` scripts are GPU-free; the `*_gui` scripts require Isaac Sim. `validate_thissio`
has a `--self-test`. `uv run pytest` runs 34 tests.
...
- `make_paper_figures.py` builds **F1** (per-site calibration), **F2** (leave-location-out,
  ...
  **F5** (convergence: lux error + render time vs totalSpp), and **F6** (M0→M4 ablation)
  from the result CSVs / monolithic dataset.
...
- `make_j_occlusion_figure.py` composes the native-Isaac illuminance figures from RTX
  `PtIlluminance` screenshots with a shared `jet` 0–35 klux colour bar (matching the Isaac
  legend, so they stay colour-harmonic with F10/F10b and F16b):
  **F7**, a single-panel rooftop lux map (Location A at solar noon), replacing the earlier
  false-colour `.npy` render; and **F9b**, the three-panel Location-J inter-building
  occlusion figure at the three low-sun timestamps that are the largest leave-location-out
  scatter outliers (qualitative evidence that the twin resolves occlusion CAMS cannot).
  **F9b replaces the former F8 (H shadow sweep)** as the geometry figure, because J is the
  genuine occlusion outlier whereas H's pixel was re-picked clean.
...
- **Render time:** the per-totalSpp render times are in `convergence_table_spp.csv`
  (about 54 s/frame at totalSpp=512 on the RTX 4070).
- The deprecated `benchmark_convergence_gui.py` does not measure convergence (render time
  is flat because rt_subframes is not the convergence knob); use the **totalSpp** table.
- VRAM is an end-of-render `nvidia-smi` snapshot (a lower bound on peak, not the true peak).
- Production runs used totalSpp = 32; the `RT_SUBFRAMES = 24` value in §8 is the earlier
  default. The convergence table covers both.
...
CAMS McClear **R² = 0.972** (CAMS leads on this near-horizontal site, as expected).
